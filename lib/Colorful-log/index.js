...
const colorfulLog = ({ usage = "log", logBy = "", logName = "log" }, ...logData) => {
  ...
  return console[usage](
    ...
    `background: ${index.DEEP_CYAN}; padding: 6px; border-radius: 1px 0 0 1px; color: #fff; font-size: 13px;`,
    `background: ${index.BLUE_GRAY}; padding: 6px; border-radius: 0 1px 1px 0; color: #fff; font-size: 13px;`,
...
class ColorfulLogClass {
  constructor(config) {
    ...
    this.preBlockColor = config.preBlockColor || index.DEEP_CYAN;
    this.nextBlockColor = config.nextBlockColor || index.BLUE_GRAY;
