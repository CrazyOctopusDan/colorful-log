...
const index = require("../styles/index.js");
...
const colorfulLog = ({ usage = "log", logBy = "", logName = "log" }, ...logData) => {
  ...
  return console[usage](
    ...
    `background: ${index.VUE_DEEP_CYAN}; padding: 6px; border-radius: 1px 0 0 1px; color: #fff; font-size: 13px;`,
    `background: ${index.VUE_BLUE_GRAY}; padding: 6px; border-radius: 0 1px 1px 0; color: #fff; font-size: 13px;`,
...
class ColorfulLogClass {
  constructor(config) {
    ...
    this.preBlockColor = config.preBlockColor || index.VUE_DEEP_CYAN;
    this.nextBlockColor = config.nextBlockColor || index.VUE_BLUE_GRAY;
    this.padding = config.padding || 6;
    this.borderRadius = config.borderRadius || 1;
    this.fontColor = config.fontColor || "#fff";
  }
}
exports.colorfulLog = colorfulLog;
exports.default = ColorfulLogClass;
exports.materialColorfulLog = materialColorfulLog;
